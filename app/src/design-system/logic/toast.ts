  ...
  transaction: {
    signing: (hint?: string) => {
      return sonnerToast.loading('Waiting for signature...', {
        description: hint ?? 'Please approve the transaction in your wallet',
        style: {
          borderColor: 'var(--trust)',
        },
      });
    ...
    /** In-place progress while reusing the same toast id. */
    phase: (id: string | number, title: string, description?: string) => {
